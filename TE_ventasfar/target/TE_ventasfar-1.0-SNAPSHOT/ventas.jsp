<%-- 
    Document   : ventas
    Created on : 25 nov. 2021, 16:00:26
    Author     : WINDOWS 10
--%>
...
<body>
    <div class="container">
        ...
        <br>
        <a href="VentaControlador?action=add" class="btn btn-primary btn-sm"><i class="fas fa-plus-circle"></i> Nuevo Registro</a>
            ...
            <tbody>
                <c:forEach var="item" items="${ventas}">
                    <tr>
                        ...
                        <td>${item.cantidad}</td>
                        <td>${item.precio}</td>
                        <td><a href="VentaControlador?action=edit&id=${item.id}" class="btn btn-warning">Editar</a></td>
                        <td><a href="VentaControlador?action=delete&id=${item.id}" onclick="return(confirm('¿Esta seguro de eliminar el registro seleccionado?'))" class="btn btn-danger">Eliminar</a></td>
                    </tr>                    
                </c:forEach>
            </tbody>
        </table>
