...
public class ClienteDAOimpl extends ConexionDB implements ClienteDAO {
    ...
    public void insert(Cliente cliente) throws Exception {
        try {
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("INSERT INTO clientes (ci,nombre,apellido,correo,celular) values (?,?,?,?,?)");
    ...
    public void update(Cliente cliente) throws Exception {
        try {
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("UPDATE clientes SET ci=?,nombre = ?,apellido=?, correo = ?, celular = ? where id = ?");
            ps.setString(1, cliente.getCi());
            ps.setString(2, cliente.getNombre());
            ps.setString(3, cliente.getApellido());
            ps.setString(4, cliente.getCorreo());
